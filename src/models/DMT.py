...
class DMT(nn.Module, ReporterMixin):
    def __init__(
        ...
        self.labeled_loader, self.unlabeled_loader = map(
            lambda data, batch_size: DataLoader(data, batch_size, shuffle=True, num_workers=4),
    ...
    def compute_weights(
        ...
        teacher_max_confidences, _ = torch.max(teacher_class_confidences, dim=-1, keepdim=True)
                ...
                for i, (unlabeled, (labeled, labels)) in enumerate(zip(self.unlabeled_loader, self.labeled_loader)):
                    ...
                    with torch.no_grad():
                        ...
                        weights = self.compute_weights(
                            pseudolabels, mask, teacher_confidences, student_confidences,
