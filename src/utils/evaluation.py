...
def evaluate_IoU(
    ...
    for images, labels in data:
        ...
        for i in range(2):
            intersection = (
                torch.logical_and(predictions == i, labels == i)
            ).sum()
            union = (torch.logical_or(predictions == i, labels == i)).sum()
            IoUs[i] += (intersection / union) * images.shape[
                0
...
def watched_evaluate_IoU(
    ...
    for images, labels in data:
        ...
        IoU = (
            intersection / union
        )  # Got this proportion correct on this batch
        ...
        worst_batch_best = [
            (idx, val)
            for idx, val in zip(worst_batch_best_idx, worst_batch_best_val)
        ]
